...
class MultiScan(nn.Module):
    ...
    # ALL_CHOICES = ('h', 'h_flip', 'v', 'v_flip')# 消融-无聚类
    def __init__(self, dim, choices=None, token_size=(14, 14),win_size=8):
        ...
        if numbers:
            ...
            self.local_clusters = nn.ModuleDict({
                str(number): LocalCluster(dim=dim, w_size=self.win_size, clusters=number)
                for number in set(int(item[1:]) for item in choices if item.startswith('c'))
            })
    ...
    def scan(self, x, direction='h'):
        ...
        if len(x.shape) == 3:
            if direction == 'h':
                ...
                K = int(direction[1:])
                return self.local_clusters[str(K)](x)
...
class LocalCluster(nn.Module):
    ...
    def cluster(self, f, v, Wg, Hg):
        ...
        sim_max, sim_max_idx = sim.max(dim=1, keepdim=True)# 每个点只保留一个中心
